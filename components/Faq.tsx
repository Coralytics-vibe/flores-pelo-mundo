...
const FaqItem: React.FC<FaqItemProps> = ({ question, answer, isOpen, onClick }) => {
  return (
    <div className="border-b border-gray-200 last:border-b-0">
      ...
      >
        <span className={`text-lg font-medium transition-colors duration-300 ${isOpen ? 'text-[#F79824]' : 'text-gray-900'}`}>{question}</span>
        <ChevronDownIcon className={`w-5 h-5 transform transition-all duration-300 ${isOpen ? 'rotate-180 text-[#F79824]' : 'text-gray-600'}`} />
      </button>
      <div
        className={`grid transition-all duration-500 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}
      >
        <div className="overflow-hidden">
          <div className="pb-5 px-6 text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: answer }} />
        </div>
      </div>
    </div>
  );
    ...
    <section id="faq" className="py-20 bg-gray-900">
      <div className="container mx-auto px-6">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-x-8 gap-y-8">
          <div className="bg-white rounded-2xl h-fit shadow-lg">
